...
%                        length( beamWidth_vector ), ...
%                        length( apDensity_vector ), ...
%                        length( apHeight_vector ), ...
%                        length( blockageDensity_vector ), ...
%                        length( bodyAttenuation_vector ) );
percentile_sinr = zeros( 3, ...
                       length( beamWidth_vector ), ...
                       length( apHeight_vector ), ...
                       length( interSiteDistance_vector ), ...
                       length( blockageDensity_vector ), ...
                ...
                for d_id = 1:length( interSiteDistance_vector )
                    ...
                    for n_iter = 1:numberOfIterations
                        ...
                        if mod(loopProgress,2) == 0
                            ...
                            disp(['Estimated end time: ' num2str(estimatedEnd) ' seconds.']);
                        end
...
%                         coverage = sum( sinr_vector > sinrThreshold ) / ...
%                             length( sinr_vector );
%                         coverage_sinr( st_id, bw_id, d_id, h_id, bl_id, ba_id ) = ...
%                             coverage;
%                     end

                    % AVERAGE SPECTRAL EFFICIENCY
                    avg_spectralEff( bw_id, h_id, d_id, bl_id, ba_id ) = ...
                        mean( spectralEff_vector );

                    % 5th PERCENTILE OF SINR
                    %   the greatest SINR value of the lowest 50%
                    percentile_sinr( 1, bw_id, h_id, d_id, bl_id, ba_id ) = ...
                        pow2db( prctile( ...
                        sinr_vector( :, bw_id, h_id, d_id, bl_id, ba_id ), ...
                        5 ) );
                    percentile_sinr( 2, d_id, h_id, bw_id, bl_id, ba_id ) = ...
                        pow2db( prctile( ...
                        sinr_vector( :, bw_id, h_id, d_id, bl_id, ba_id ), ...
                        50 ) );
                    percentile_sinr( 3, bw_id, h_id, d_id, bl_id, ba_id ) = ...
                        pow2db( prctile( ...
                        sinr_vector( :, bw_id, h_id, d_id, bl_id, ba_id ), ...
                        95 ) );
